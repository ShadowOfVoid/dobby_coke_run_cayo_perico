...
Citizen.CreateThread(function()
    ...
    ESX.TriggerServerCallback('dobby_coke:processcoords', function(servercoords)
...
Citizen.CreateThread(function()
	while ESX == nil do TriggerEvent('esx:getSharedObject', function(obj) ESX = obj end) Wait(0) end
    ESX.TriggerServerCallback('dobby_coke:startcoords', function(servercoords)
        coord = servercoords
	end)
end)
		...
		if not inUse then
			if dist <= 1 then
				...
				if IsControlJustPressed(1, 51) then
					ESX.TriggerServerCallback('dobby_coke:pay', function(success)
						if success then
							main()
						end
					end)
...
RegisterNetEvent('dobby_coke:syncTable')
AddEventHandler('dobby_coke:syncTable', function(bool)
    inUse = bool
end)

RegisterNetEvent('dobby_coke:onUse')
AddEventHandler('dobby_coke:onUse', function(source)
	if Config.useMythic then
		exports['mythic_notify']:DoLongHudText('inform', _U'dobby_coke_use')
	end
	local crackhead = GetPlayerPed(-1)
	SetPedArmour(crackhead, 30)
	SetTimecycleModifier("DRUG_gas_huffin")
	Citizen.Wait(Config.dobby_cokeTime)
	DoScreenFadeOut(1000)
	Citizen.Wait(1000)
	DoScreenFadeIn(2000)
	if Config.useMythic then
		exports['mythic_notify']:DoLongHudText('inform', _U'dobby_coke_off')
	end
...
function main()
	TriggerServerEvent('dobby_coke:updateTable', true)
...
function planeGround()
	local planehash = GetHashKey("mammatus")
	...
	while checkPlane do
		...
		if DoesEntityExist(airplane) then
			if GetVehicleEngineHealth(airplane) < 0 then
				...
				TriggerServerEvent('dobby_coke:updateTable', false)
				checkPlane = false
			end
		else
			sleep = 3000
		...
		while hangar do
			...
			if disttocoord <= 5 then
				...
				if IsControlJustPressed(1, 51) then
					...
					Citizen.Wait(2000)
					TriggerServerEvent('dobby_coke:GiveItem')
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if dist <= 3 and not isProcessing then
			sleep = 5
			DrawText3Ds(process.x, process.y, process.z, _U'break_dobby_coke')			
			if IsControlJustPressed(1, 51) then		
				isProcessing = true
				ESX.TriggerServerCallback('dobby_coke:process', function(success)
					if success then					
						processing()
					else
						isProcessing = false
...
function processing()
	...
	FreezeEntityPosition(player, true)
	if Config.progBar then
		exports['progressBars']:startUI(6000, _U'breaking_dobby_coke')
	end
	playAnim("anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 6000)
	Citizen.Wait(6000)
	FreezeEntityPosition(player, false)
	TriggerServerEvent('dobby_coke:processed')
	isProcessing = false
end
